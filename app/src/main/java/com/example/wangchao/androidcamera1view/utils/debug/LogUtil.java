...
public class LogUtil {
    ...
    public static boolean isLoggable(Tag tag, int level) {
        ...
        if (getOverrideLevelFromProperty() > -1) {
            ...
            checkLevelResult = getLogLevelFromSystemLevel(level)
                    <= getOverrideLevelFromProperty();
        }
        shouldLog(tag, level);
    ...
    private static int getOverrideLevelFromProperty() {
        ...
        try {
            logLevelFromProperties = 0;//SystemProperties.getInt("debug.mtkcam.loglevel", MTKCAM_LOG_LEVEL_DEFAULT);
